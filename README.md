...
본 리포지토리는 단순한 수익률 예측이 아니라 **시장 구조(market structure) 조건**에 기반하여, 포트폴리오의 위험 노출을 동적으로 조정하는 시스템 기반 멀티자산 전략을 제시합니다.
...
본 전략의 핵심 목표는, 특히 포지션 쏠림(크라우딩)이나 유동성 악화로 인해 기존 시그널이 실패하는 구간에서 **낙폭(drawdown) 관리와 국면(regime) 전반의 견고함**을 확보하는 것입니다.
...
- 변동성이 상승할수록 포트폴리오 익스포저를 축소
- 불안정 국면에서 과도한 리스크 노출을 방지

### 3. 크라우딩 오버레이
- 자산 간 롤링 상관계수를 사용해 시장 수준의 크라우딩을 측정
- 상관계수 상승은 포지션 쏠림 및 분산 효과 약화를 의미
- 상관 구조가 악화되는 국면에서 익스포저를 축소

### 4. 유동성 오버레이
- Amihud 방식의 비유동성(illiquidity) 프록시로 유동성 수준 추정
- 유동성 스트레스 구간에서 익스포저를 축소하도록 설계
- 급락 리스크 및 강제 디레버리징 시나리오에 대응

### 5. 현금 배분 (BIL)
- 잔여 비중은 단기 국채 ETF(BIL)에 배분
- 현금을 명시적인 포트폴리오 구성 요소로 취급
- 시그널이 유효하지 않거나 확보되지 않을 경우 전액 현금으로 폴백
...
- 성과 평가를 위해 포트폴리오 비중은 일 단위로 forward-fill 적용
- 레버리지 및 공매도는 가정하지 않음
- 총 익스포저는 100%로 제한
...
본 전략은 상승장에서의 일부 수익 참여를 희생하는 대신, **낙폭과 리스크 조정 성과를 유의미하게 개선**하는 것을 목표로 합니다.
